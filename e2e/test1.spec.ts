...
const executeTransfer = async (page: Page, btn: Locator, amount = "5") => {
    ...
    await new Promise((resolve) => {
        ...
        // Click button after lisenter is setup
        btn.click();
    });
...
test.describe("Upgrades & token transfer flow", () => {
    ...
    test.describe("IC TOKENS", () => {
        ...
        test("Add - select", async () => {
            await expect(
                page.getByTestId("bkyz2-fmaaa-aaaaa-qaaaq-cai-balance"),
            ).not.toBeVisible(); // List is empty

            const tokenSelectBtn = page.getByTestId(
                "icrc-wallet-token-selector",
            );
            expect(tokenSelectBtn).toBeVisible();

            await page.selectOption(
                'select[data-testid="icrc-wallet-token-selector"]',
                { index: 0 },
            ); // Selects the first option

            await expect(
                page.getByTestId("bkyz2-fmaaa-aaaaa-qaaaq-cai-balance"),
            ).toBeVisible(); // Token added to the list
        });
    });
});
