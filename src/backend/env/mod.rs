...
impl State {
    ...
    pub fn vote_on_report(&mut self, principal: Principal, post_id: PostId, vote: bool) {
        let user = self
            .principal_to_user(principal)
        ...
        let report = match post.vote_on_report(stalwarts, user.id, vote) {
            Some(report) if report.closed => report,
            _ => return,
        };
        ...
        let (sponsor_id, unit) = if report.confirmed_by.len() > report.rejected_by.len() {
            ...
            let post_author = self.users.get_mut(&post.user).expect("no user found");
            ...
            let reporter = self.users.get_mut(&report.reporter).expect("no user found");
        ...
        let stalwarts = report
            ...
            .collect::<Vec<_>>();
        let stalwart_reward =
            (unit / stalwarts.len() as i64).min(CONFIG.stalwart_moderation_reward);
        let mut total_stalwart_rewards = 0;
        let log = "moderation rewards for stalwarts";
        for stalwart_id in stalwarts.iter() {
            let moderator = self.users.get(stalwart_id).expect("no user found").id;
            self.transfer(
                sponsor_id,
                moderator,
                stalwart_reward,
...
pub(crate) mod tests {
    ...
    fn test_reporting() {
        ...
        state.vote_on_report(pr(3), post_id, true);
        let p = state.posts.get(&post_id).unwrap();
        let report = &p.report.clone().unwrap();
        ...
        state.vote_on_report(pr(6), post_id, false);
        let p = state.posts.get(&post_id).unwrap();
        let report = &p.report.clone().unwrap();
        assert_eq!(report.confirmed_by.len(), 1);
        ...
        state.vote_on_report(pr(12), post_id, true);
        let p = state.posts.get(&post_id).unwrap();
        let report = &p.report.clone().unwrap();
        ...
        state.vote_on_report(pr(13), post_id, true);
        let p = state.posts.get(&post_id).unwrap();
        let report = &p.report.clone().unwrap();
        ...
        state.vote_on_report(pr(9), post_id, false);
        let p = state.posts.get(&post_id).unwrap();
        let report = &p.report.clone().unwrap();
        assert_eq!(report.confirmed_by.len(), 0);
        assert_eq!(report.rejected_by.len(), 2);

        state.vote_on_report(pr(10), post_id, false);
        let p = state.posts.get(&post_id).unwrap();
        let report = &p.report.clone().unwrap();
        assert_eq!(report.confirmed_by.len(), 0);
