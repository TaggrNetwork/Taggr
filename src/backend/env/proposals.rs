...
pub fn create_proposal(
    ...
    let required_balance = state.tokens_locked_on_active_proposals(caller) + escrow_tokens;
...
pub mod tests {
    ...
    fn test_token_locks() {
        mutate(|state| {
            ...
            // Set some random market price and xdr rate;
            // This will require ~5 tokens for one proposal.
            state.auction.last_auction_price_e8s = 18625300;
            state.e8s_for_one_xdr = 12345678;
            ...
            assert_eq!(
                ...
                Err("token balance required for a new proposal: 5".into())
            );
            // Insert 7 tokens and make sure only one proposal can be created.
            let tokens = 700;
            ...
            assert_eq!(
                ...
                Err("token balance required for a new proposal: 10".into())
            );

            // Make sure you can't transfer more than 2 tokens
    ...
    fn test_controversial_rejection() {
        mutate(|state| {
            ...
            // Now let the proposer approve it own proposal, but everyone else reject it.
            // Since 4 users own 400 tokens and the whale owns another 400 (50% of the supply),
            // he should still be penalized for controversial proposal if 33% of vp rejects their
            // proposal.
