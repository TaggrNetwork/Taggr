...
async fn post_upgrade_fixtures() {
    // Mints tokens that were skipped during the last distribution due to a bug: https://taggr.link/post/34188
    for (user_id, tokens) in vec![
        (0, 59),
        (8, 148),
        (34, 6),
        (140, 8),
        ...
        mutate(|state| {
            let acc = account(state.users.get(&user_id).unwrap().principal);
            crate::token::mint(state, acc, tokens * 100);
        })
    }
}
