    ...
    return <>
        ...
        <div className="spaced">
            ...
            <FileUploadInput callback={async binary => {
                if(!confirm("Do you really want to upload a new binary? This will reset all existing votes.")) return;
            ...
            {emergency_votes.length > 0 && <>
                ...
                <ul className="monospace">
                    {emergency_votes.map(id => <li key={id}>{id}</li>)}
                </ul>
            </>}
            {api._user && !emergency_votes.includes(api._user.principal) && <>
                ...
                <ButtonWithLoading onClick={async () => {
                    await api.call("confirm_emergency_release", hash);
                    alert("Thanks!");
                    location.reload();
                }} label="SUBMIT" />
            </>}
