...
export const Feed = ({params, heartbeat}) => {
    ...
    return <div className="column_container">
...
const FeedExtender = ({filterVal, setFilterVal, refilter, filter}) => {
    ...
    return <div className="top_half_spaced row_container flex_ended">
        {extending && <div className="row_container max_width_col">
            <input type="text" className="medium_text max_width_col" 
                vlaue={filterVal} onChange={e => setFilterVal(e.target.value)}
                placeholder="Enter @user or #tag" />
            <button className="right_half_spaced" onClick={() => { refilter(); setExtending(false) }}>DONE</button>
        </div>}
        {!extending && <button className="max_width_col right_half_spaced" onClick={() => setExtending(!extending)}>EXTEND</button>}
        {!extending && api._user && <ToggleButton classNameArg="max_width_col left_half_spaced"
            currState={() => contains(api._user.feeds, filter)}
            toggler={() => api.call("toggle_following_feed", filter).then(api._reloadUser)} />}
    </div>;
}
...
const FeedBar = ({params, callback}) => {
    ...
    const renderToken = token => token.startsWith("@")
        ? <a key={token} className="tag" href={`#/user/${token.replace("@", "")}`}>{token}</a>
        : <a key={token} className="tag" href={`#/feed/${token}`}>#{token}</a>;

    filter.sort();
    setTitle(`feed: ${filter.join(" + ")}`);
    return <HeadBar 
        title={filter.map(renderToken).reduce((prev, curr) => [prev, " + ", curr])}
        shareLink={`feed/${filter.join("+")}`} shareTitle={`Hash-tag feed on ${backendCache.name}`}
        content={<FeedExtender filterVal={filterVal} setFilterVal={setFilterVal} filter={filter} refilter={refilter} />} 
        menu={true}
    />;
};
