import {HeadBar, Loading, ButtonWithLoading, timeAgo, token, userList, percentage, FileUploadInput, tokenBalance} from "./common";
...
export const Proposals = () => {
    ...
    return <>
        ...
        <div className="vertically_spaced">
            {currentMask == "reward" && <div className="spaced column_container monospace">
                <div className="vcentered bottom_half_spaced">RECEIVER<input type="text" className="monospace left_spaced max_width_col" onChange={async ev => { setReceiver(ev.target.value); }} /></div>
                ...
                <ButtonWithLoading classNameArg="active" onClick={async () => {
                    ...
                    let response = await api.call("propose_reward", description, receiver);
                    if ("Err" in response) {
            ...
            {currentMask == "funding" && <div className="spaced column_container monospace">
                ...
                {description && <Content value={description} preview={true} classNameArg="bottom_spaced framed" />}
                <ButtonWithLoading classNameArg="active" onClick={async () => {
                    if (!description || !receiver || !fundingAmount) {
                        alert("Error: incomplete data.");
                        return;
                    }
                    let response = await api.call("propose_funding", description, receiver, parseInt(fundingAmount));
                    if ("Err" in response) {
                        alert(`Error: ${response.Err}`);
                        return;
                    }
            ...
            {currentMask == "release" && <div className="spaced column_container monospace">
...
export const Proposal = ({id}) => {
    ...
    const vote = async (proposal, adopted) => {
        ...
        if (adopted) {
            ...
            if ("Reward" in proposal.payload) {
                data = prompt("Please enter the token amount which would be an appropriate reward for the efforts described:");
                if (!data) return;
                if(!confirm(`You vote for issuing the reward of ${data} tokens.`)) return;
            }
        }
        const result = await api.call("vote_on_proposal", proposal.id, adopted, data);
        if ("Err" in result) {
    ...
    return <div key={proposal.timestamp} className="post_extension column_container monospace">
        ...
        {!!proposal.payload.Fund && <>
            ...
            <div className="bottom_spaced">AMOUNT: {tokenBalance(proposal.payload.Fund[1])}</div>
        </>}
