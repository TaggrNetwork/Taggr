...
LOG=/app/test-results/release.log
mkdir -p "$(dirname "$LOG")"
: > "$LOG"
...
prepare_artifacts() {
  ...
  NODE_ENV=production npm run build --quiet >>"$LOG" 2>&1
  ./build.sh bucket >>"$LOG" 2>&1
  ./build.sh taggr >>"$LOG" 2>&1
}
...
run_e2e() {
  ...
  # with `bash -x` or remove the `>>"$LOG" 2>&1` redirects to debug.
  echo "==> [4/7] e2e: dfx start + ICP ledger + canister create"
  dfx start --background -qqqq >>"$LOG" 2>&1
  ./e2e/import_local_minter.sh >>"$LOG" 2>&1
  dfx canister create --all >>"$LOG" 2>&1
  ./e2e/install_icp_ledger.sh >>"$LOG" 2>&1

  echo "==> [5/7] e2e: dev build (needs .dfx/local/canister_ids.json from create)"
  NODE_ENV=production DFX_NETWORK=local npm run build --quiet >>"$LOG" 2>&1
  ./build.sh bucket >>"$LOG" 2>&1
  FEATURES=dev ./build.sh taggr >>"$LOG" 2>&1

  echo "==> [6/7] e2e: deploy + cycles"
  FEATURES=dev dfx deploy taggr >>"$LOG" 2>&1
  dfx --identity local-minter ledger fabricate-cycles --all --cycles 1000000000000000 >>"$LOG" 2>&1
